...
/**
 * The language picker's modal, split from its button so that `motion` — 144kB
 * of it — is not on the landing page's critical path. The top bar's button is
 * the only reason the entry chunk pulled the animation library at all; here it
 * arrives with the first open, by which time the timeline stage has usually
 * fetched it anyway.
 *
 * Stays mounted (with `isOpen` false) once it has been opened, so AnimatePresence
 * still gets to run its exit animation.
 */
